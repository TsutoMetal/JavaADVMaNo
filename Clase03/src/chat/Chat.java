...
public class Chat extends javax.swing.JFrame {
    public Chat() {
        initComponents();
        ListaDirecciones.getMapa().keySet().forEach(cmbLista::addItem);
    ...
    private void btnEnviarActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnEnviarActionPerformed
        ...
        try (
                Socket so=new Socket(
                        ListaDirecciones.getMapa().get(cmbLista.getSelectedItem())
                        ,7000
                );
                OutputStream out=so.getOutputStream();
            ) {
            out.write(txtMensaje.getText().getBytes());
            txaChat.append("Mensaje enviado a "+cmbLista.getSelectedItem()+".\n");
            txtMensaje.setText("");
